...
PendingOrder mk_raw(double stop, double limit, bool is_long) {
    ...
    o.is_long = is_long;
    o.legs.set_stop_price(stop);
    o.legs.set_limit_price(limit);
    o.legs.set_trail_points(kNaN);
    o.legs.set_trail_offset(kNaN);
    o.qty = kNaN;
    o.qty_type = -1;
    o.qty_percent = 100.0;
    o.oca_type = 0;
...
PendingOrder mk_exit(double stop, double limit, double trail_points,
    ...
    o.legs.set_stop_price(stop);
    o.legs.set_limit_price(limit);
    o.legs.set_trail_points(trail_points);
    o.legs.set_trail_offset(trail_offset);
    o.qty = kNaN;
    o.qty_type = -1;
    o.qty_percent = 100.0;
    o.oca_type = 0;
...
static void test_resolve_exit_trail_fills() {
    ...
    // at the open (exits-at-activation gap rule; no pre-arming involved —
...
static void test_zero_offset_trail_arms_from_the_carried_best() {
    ...
    // TV fills at that open — only a carried armed state produces this.
    ...
    // Control: an OFFSET trail keeps the carried-best arming — activation is
