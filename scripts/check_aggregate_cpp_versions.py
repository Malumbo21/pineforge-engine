...
def check_texts(files):
    ...
    if namespaces != ["engine_script_run_v11", "engine_script_run_v11"]:
        raise ValueError("PendingOrder/BacktestEngine require engine_script_run_v11")
    ...
    if not re.match(r'\s*Fnv\s+f;\s*f\.s\("pineforge-broker-state/v11"\);', broker):
        raise ValueError("broker entry requires v11 domain")
    ...
    if compact.count(fold) != 1:
        raise ValueError("stream entry requires v11 then broker hash")
    prefix = compact[:compact.index(fold)]
    if prefix.count('{') != prefix.count('}') or (prefix and prefix[-1] not in ';}'):
        raise ValueError("stream v11 fold must be unconditional")
    ...
    for name, text in files.items():
        if name.startswith("include/pineforge/compat/pine/"):
            ...
            if any(value != "engine_script_run_v11" for value in found):
