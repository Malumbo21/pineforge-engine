...
class PhysicalLotCoverage(unittest.TestCase):
    ...
    def test_layout_and_hash_versions_must_match_v11_contract(self):
        self.assertIn("engine_script_run_v11", HEADER)
        self.assertIn('f.s("pineforge-broker-state/v11");', SOURCE)
        self.assertIn("integer(11); integer(broker_state_hash());", STREAM)
        self.assertEqual(self.check(header=HEADER.replace("engine_script_run_v11", "engine_script_run_v10"))[0], 1)
        for replacement in ['f.s("pineforge-broker-state/v10");', '',
                            '// f.s("pineforge-broker-state/v11");']:
            self.assertEqual(self.check(source=SOURCE.replace(
                'f.s("pineforge-broker-state/v11");', replacement))[0], 1)
        for replacement in ["integer(10); integer(broker_state_hash());",
                            "integer(broker_state_hash());",
                            "if (false) { integer(11); integer(broker_state_hash()); }"]:
            self.assertEqual(self.check(stream=STREAM.replace(
                "integer(11); integer(broker_state_hash());", replacement))[0], 1)

    def test_version_folds_in_unrelated_helpers_do_not_cover_entry_points(self):
        broker_fold = 'f.s("pineforge-broker-state/v11");'
        altered = SOURCE.replace(broker_fold, '') + '\nvoid other() { ' + broker_fold + ' }\n'
        self.assertEqual(self.check(source=altered)[0], 1)
        stream_fold = "integer(11); integer(broker_state_hash());"
