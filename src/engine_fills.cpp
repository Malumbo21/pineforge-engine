...
void BacktestEngine::process_pending_orders(const Bar& bar, bool before_pooc_script) {
    ...
    if (position_side_ == PositionSide::FLAT) {
        ...
        // -- and reset once per BAR (not per pass), at every dispatch site
        // that owns a bar boundary: dispatch_bar() and
        // run_aggregation_bar_loop's magnifier branch (engine_run.cpp),
        // reset_run_state() (engine_run.cpp, so a rerun that dispatches zero
        // script bars doesn't leak the prior run's decision), and
        // stream_dispatch_script_bar() (engine_stream.cpp, which calls
        // process_pending_orders() directly and never goes through
        // dispatch_bar()). So it survives this pass's own dual_entry_path_
        // going back to None.
...
void BacktestEngine::process_margin_call(const Bar& bar) {
    ...
    // round 8 family R / round 10 family AB: it does carry TradingView's
    // 10-significant-digit trigger (tv_money_long_margin_call) — the broker
    // marks the position value on rounded money, so a long whose free cash is
    // below the rounding residual is liquidated one contract at the first
    // such path point.
...
void BacktestEngine::apply_filled_order_to_state(
    ...
    if (order.type == OrderType::MARKET
        ...
        if (margin_pct > 0.0) {
            ...
            // Explicit fixed quantities use the same monetary admission
            // checks as default all-in sizing: the unrounded placement
            // balance must cover the rounded signal cost, then the price
            // affordable from that balance must reach the signal price.
            // Both use the actual lot-floored contracts. Fees remain a
            // separate post-fill concern; the exact signal/fill cost check
            // follows. A lower opening price does not rescue these checks.
            ...
            const bool explicit_money_scope =
